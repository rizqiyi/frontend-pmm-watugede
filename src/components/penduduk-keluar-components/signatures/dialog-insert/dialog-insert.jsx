...
import { postSignaturePendudukKeluar } from "../../../../reducers/signatures/signatures.actions";
...
const DialogInsertSignatureComponent = ({ ...props }) => {
  const { open, handleClose, postSignaturePendudukKeluar, id } = props;
      ...
      >
        <Box p={3}>
          <Formik
            ...
            onSubmit={(values) => {
              postSignaturePendudukKeluar(values, values.id);
              handleClose(false);
            }}
          >
            {() => (
...
const mapDispatchToProps = (dispatch) => {
  return {
    postSignaturePendudukKeluar: (requests, id) =>
      dispatch(postSignaturePendudukKeluar(requests, id)),
  };
};
